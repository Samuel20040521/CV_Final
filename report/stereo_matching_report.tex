...
\begin{figure}[H]\centering
  \begin{subfigure}[t]{0.48\linewidth}\centering
    \includegraphics[width=\linewidth]{walkthrough_v6.png}
    \caption{v6 古典 baseline 重建}\end{subfigure}\hfill
  \begin{subfigure}[t]{0.48\linewidth}\centering
    \includegraphics[width=\linewidth]{walkthrough_fs.png}
    \caption{FoundationStereo 重建（zero-shot）}\end{subfigure}
...
\paragraph{SOTA 對照延伸。} §\ref{sec:bonus-sota} 進一步以
NVIDIA FoundationStereo~\cite{Wen2025}（CVPR 2025 Best Paper Nomination）
zero-shot 同 pipeline 對照，把 1\,px BPR 從 20.21\,\% 降到 3.65\,\%，
量化證實 v6 的數字是\textbf{古典演算法的結構性上限}而非實作問題；同時也說明
本專案的下游模組（NED→CV pose 修正、TSDF fusion、follow-trajectory render）
具備 matcher-agnostic 的設計，可平移到任意深度立體匹配器之上。
...
（reduce mesh noise）、以及把 v8 / FoundationStereo 的 sub-pixel float disparity
直接送進 TSDF（取代目前 \texttt{uint8} 介面）以充分利用 deep matcher 的
sub-pixel 訊號；長期則可試 Selective-Stereo~\cite{Wang2024Selective} /
IGEV++~\cite{Xu2025} 等 SceneFlow 訓練版本在 TartanAir 上的 domain gap。
